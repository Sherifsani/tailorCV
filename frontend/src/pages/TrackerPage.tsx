...
function AppCard({ app, onDelete }: { app: Application; onDelete: () => void }) {
  ...
  return (
    <div
      ...
      className={cn(
        'bg-background border rounded-lg p-3 cursor-grab active:cursor-grabbing shadow-sm hover:shadow-md transition-all select-none',
...
function Column({
  ...
  return (
    <div className={cn('flex flex-col border-t-4 rounded-xl bg-muted/20 min-w-[220px]', col.color)}>
      <div className="flex items-center gap-2 px-3 py-2.5 border-b bg-muted/30 rounded-t-xl">
        <span className={cn('w-2 h-2 rounded-full shrink-0', col.dot)} />
        <span className="text-xs font-semibold">{col.label}</span>
        <span className="ml-auto text-[10px] text-muted-foreground bg-background border rounded-full w-5 h-5 flex items-center justify-center font-medium">
          {apps.length}
        </span>
      </div>
      <div
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">{apps.length} applications tracked</p>
        </div>
        <button
      ...
      <div className="overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0">
        <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd}>
          ...
          <DragOverlay>
            {activeApp && (
              <div className="bg-background border rounded-lg p-3 shadow-xl rotate-1 opacity-95 cursor-grabbing w-52">
                <p className="text-sm font-semibold">{activeApp.company}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{activeApp.jobTitle}</p>
              </div>
            )}
          </DragOverlay>
        </DndContext>
      ...
      {apps.length === 0 && (
        <div className="text-center py-12 text-muted-foreground border-2 border-dashed rounded-xl">
          ...
          <p className="text-xs mt-1">Tailor a resume and save it here, or add one manually.</p>
        </div>
      )}
    </div>
  );
